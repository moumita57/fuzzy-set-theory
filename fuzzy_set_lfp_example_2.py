...
This file contains solution of fuzzy multiobjective linear fractional programming problem(Part 2)
"""
...
n1 = LpVariable("variable 5", 0, None, LpInteger)
n2 = LpVariable("variable 6", 0, None, LpInteger)

# Create maximize objective function
model += n1 + n2
# Create fourteen  constraints
model += 1 * y1 + 1 * y2 + 3 * t <= 5.00, "C1"
model += 1 * y1 + 1 * y2 + 3 * t - 3.68 * n1 >= 1.32, "C2"
model += 7 * y1 + 2 * y2 <= 35.00, "C3"
model += 7 * y1 + 2 * y2 - 33.6 * n2 >= 1.40, "C4"
